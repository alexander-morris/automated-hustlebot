...
class ImageMatcher:
    def __init__(self, threshold=0.75):
        ...
        # Updated thresholds with more emphasis on structural similarity
        self.quality_thresholds = {
            'confidence': 0.8,  # Increased from 0.75
            'structural_similarity': 0.7,  # Increased from 0.4
            'edge_similarity': 0.3,  # Increased from 0.15
            'pixel_difference': 0.6,  # Increased from 0.05
            'histogram_similarity': 0.85,  # Increased from 0.8
            'consensus_count': 1
        }
    ...
    def calculate_match_quality(self, region: np.ndarray, target: np.ndarray) -> MatchQuality:
        """Calculate various quality metrics for a match with emphasis on structural features."""
        try:
            ...
            _, region_binary = cv2.threshold(region_gray, 200, 255, cv2.THRESH_BINARY)
            _, target_binary = cv2.threshold(target_gray, 200, 255, cv2.THRESH_BINARY)
            ...
            if len(region_contours) > 0 and len(target_contours) > 0:
                ...
                area_diff = min(region_area, target_area) / max(region_area, target_area)
            else:
                area_diff = 0
            ...
            # Weight the structural similarity more heavily
            weighted_ssim = ssim * 0.6 + area_diff * 0.4
